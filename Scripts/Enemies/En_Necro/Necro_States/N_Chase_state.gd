...
@onready var attack_collider_position = $"../..".attack_collider_position # Collider Position
...
func chase(delta):
	...
	if not Body.is_on_floor():
		...
		if direction > 0:
			...
			attack_collider.position.x = attack_collider_position
			fire.position.x = fire_position
		elif direction < 0:
